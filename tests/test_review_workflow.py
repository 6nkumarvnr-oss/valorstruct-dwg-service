...
def test_list_review_questions_supports_pagination(monkeypatch):
    rows = [_make_row(id=1), _make_row(id=2), _make_row(id=3)]
    _patch_service_store(monkeypatch, rows)

    result = services.list_review_questions(exam_code="PMP", limit=2, offset=1)
    assert len(result) == 2
    assert result[0]["id"] == 2


def test_edit_question_fails_for_invalid_correct_answer(monkeypatch):
    row = _make_row()
    _patch_service_store(monkeypatch, [row])
    payload = QuestionEditRequest(
        question_text="Updated text",
        options=[QuestionOption(label="A", text="A"), QuestionOption(label="B", text="B")],
        correct_answer="Z",
        explanation="Updated explanation",
    )

    with pytest.raises(ValueError):
        services.edit_question(1, payload)
